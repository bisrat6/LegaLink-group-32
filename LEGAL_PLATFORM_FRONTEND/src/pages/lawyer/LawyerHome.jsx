  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch available cases (for lawyers)
        ...
        const pendingBids = bidsData.bids
          ? bidsData.bids.filter((bid) => bid.status === "pending").length
          : 0;

        // Fetch unread messages count (using notifications as proxy)
        const unreadMsgResponse = await fetch(
        ...
        const unreadMsgData = await unreadMsgResponse.json();

        // Fetch appointments for lawyer
        ...
        const upcoming = allAppointments
          ...
          .map((app) => ({
            id: app._id,
            client: app.client?.username || "Unknown Client",
            date: app.date,
            status: app.status,
          }));
        setUpcomingAppointments(upcoming);

        // Active cases (cases assigned to this lawyer)
        const activeCasesCount = casesData.cases
          ? casesData.cases.filter(
              (caseItem) =>
                caseItem.lawyer &&
                caseItem.lawyer._id && // lawyer assigned
                caseItem.lawyer._id === casesData.userId && // compare with logged-in lawyer id if available
                caseItem.status &&
        ...
        setStats({
          ...
          unreadMessages: unreadMsgData.count || 0,
          pendingBids: pendingBids,
          totalEarnings: 0, // Replace with real data if you have an endpoint
          clientsHelped: 0, // Replace with real data if you have an endpoint
        });

        // Legal updates (still mock)
        setLegalUpdates([
          {
            id: 1,
